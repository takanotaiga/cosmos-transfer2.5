...
export OUTPUT_DIR=outputs/robot_multicontrol
...
torchrun --nproc_per_node=$NUM_GPUS --master_port=12345 examples/inference.py \
  --num_gpus $NUM_GPUS \
  --params_file assets/robot_example/multicontrol/robot_multicontrol_spec.json
...
export OUTPUT_DIR=outputs/car_multicontrol
export GENERATED_OUTPUT_FILE=$OUTPUT_DIR/output.mp4
NUM_GPUS=8
torchrun --nproc_per_node=$NUM_GPUS --master_port=12345 examples/inference.py \
  --num_gpus $NUM_GPUS \
  --params_file assets/car_example/multicontrol/car_multicontrol_spec.json
