...
export OUTPUT_DIR=outputs/robot_seg
...
torchrun --nproc_per_node=$NUM_GPUS --master_port=12345 examples/inference.py \
  --num_gpus $NUM_GPUS \
  --params_file assets/robot_example/seg/robot_seg_spec.json
...
export OUTPUT_DIR=outputs/car_seg
export GENERATED_OUTPUT_FILE=$OUTPUT_DIR/output.mp4
NUM_GPUS=1
torchrun --nproc_per_node=$NUM_GPUS --master_port=12345 examples/inference.py \
  --num_gpus $NUM_GPUS \
  --params_file assets/car_example/seg/car_seg_spec.json
