...
class MultiviewControlVideo2WorldConfig(ControlVideo2WorldConfig):
...
class MultiviewControlVideo2WorldModel(ControlVideo2WorldModel):
    def __init__(self, config: MultiviewControlVideo2WorldConfig, *args, **kwargs):
        ...
        super(ControlVideo2WorldModel, self).__init__(config, *args, **kwargs)
        if config.train_base_model:
            log.warning("MultiviewControlVideo2WorldModel: Training base model")
        else:
            log.warning("MultiviewControlVideo2WorldModel: Freezing base model")
    ...
    def sample_first_n_views_from_data_batch(self, data_batch, n_views):
        ...
        for key in [NUM_CONDITIONAL_FRAMES_KEY, USE_APG_KEY, CONTROL_WEIGHT_KEY]:
