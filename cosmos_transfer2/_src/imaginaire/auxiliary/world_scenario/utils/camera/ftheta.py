...
class FThetaCamera(CameraBase):
    ...
    def to(
        ...
        if device is None:
            ...
            if isinstance(device, torch.device):
                ...
                if device.index is not None:
                    target_device = f"{target_device}:{device.index}"
            else:
        ...
        poly = np.array(intrinsics[4:10], dtype=np.float32)
        is_bw_poly = bool(int(intrinsics[10]))
        linear_cde = np.array(intrinsics[11:14], dtype=np.float32)
    ...
    @classmethod
    def from_numpy(cls, intrinsics: np.ndarray, device: Optional[str] = None) -> "FThetaCamera":
    ...
    def __init__(
        ...
        linear_cde: Optional[np.ndarray] = None,
    ...
    def _cache_torch_tensors(self) -> None:
        ...
        # Cache backward polynomial for pixel2ray
        self._bw_poly_torch = torch.tensor(self._bw_poly.coef, dtype=self.dtype, device=self.device)
        self._bw_poly_powers_torch = torch.arange(len(self._bw_poly_torch), dtype=self.dtype, device=self.device)

    def rescale(self, ratio: float | None = None, ratio_h: float | None = None, ratio_w: float | None = None) -> None:
        """
        Rescale the camera with specified ratios.

        Args:
            ratio: Uniform scaling ratio for both dimensions (if rescale to a smaller image, ratio < 1)
            ratio_h: Height scaling ratio (overrides ratio if provided)
            ratio_w: Width scaling ratio (overrides ratio if provided)

        Note: Either provide 'ratio' for uniform scaling, or 'ratio_h' and/or 'ratio_w' for
              non-uniform scaling. If only one of ratio_h/ratio_w is provided, it will be
              used for both dimensions.
        """
        # Handle parameter combinations
        if ratio is not None:
            # Uniform scaling with single ratio
            ratio_h = ratio
            ratio_w = ratio
        elif ratio_h is not None or ratio_w is not None:
            # Non-uniform scaling
            if ratio_h is None:
                ratio_h = ratio_w
            if ratio_w is None:
            ...
            raise ValueError("Must provide either 'ratio' or at least one of 'ratio_h'/'ratio_w'")

        assert ratio_h is not None
        assert ratio_w is not None

        # Update dimensions
        self._width = int(self._width * ratio_w)
        self._height = int(self._height * ratio_h)

        # Update optical center
        self._center[0] = self._center[0] * ratio_w
        self._center[1] = self._center[1] * ratio_h

        # For non-uniform scaling, we use the average ratio for polynomial coefficients
        # This is an approximation that works well in practice for small aspect ratio changes
        avg_ratio = np.sqrt(ratio_w * ratio_h)

        # update poly. if avg_ratio = 0.5, bw_poly_coef[i] = bw_poly_coef[i] * (2 ** i)
        if self.reference_poly == "bw":
            bw_poly_coef = self._bw_poly.coef.copy()
            for i in range(len(bw_poly_coef)):
                bw_poly_coef[i] = bw_poly_coef[i] * (1 / avg_ratio) ** i
    ...
    def _get_rays_impl(self) -> torch.Tensor:
        ...
        v y (down)
        """
        u = torch.arange(self.width, dtype=self.dtype, device=self.device)
        v = torch.arange(self.height, dtype=self.dtype, device=self.device)
    ...
    def pixel2ray_torch(self, pixels: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # Ensure input shape is (n_points, 2)
        if pixels.dim() == 1:
        ...
        rays = torch.empty(pixels.shape[0], 3, dtype=self.dtype, device=self.device)

        # For valid points calculation rays
        ...
        # For invalid points set to (0,0,1)
        rays[~valid, 0] = 0
        rays[~valid, 1] = 0
        rays[~valid, 2] = 1
    ...
    def get_xy_and_depth_batch_torch(
        self,
        points_in_world: torch.Tensor,
        camera_pose: torch.Tensor,
    ) -> torch.Tensor:
        """
        Optimized GPU batch version of get_xy_and_depth.

        Args:
            points_in_world: (M, 3) points in world coordinates (should already be on GPU)
            camera_pose: (4, 4) camera to world transformation matrix (should already be on GPU)

        Returns:
            xy_and_depth: (M, 3) where columns are [x, y, depth]
        """
        # Ensure tensors are on the same device as the camera model
        # Only move if necessary to avoid redundant transfers
        if points_in_world.device != self.device:
            points_in_world = points_in_world.to(device=self.device, dtype=self.dtype)
        if camera_pose.device != self.device:
            camera_pose = camera_pose.to(device=self.device, dtype=self.dtype)

        # Compute world to camera transform
        world_to_camera = torch.linalg.inv(camera_pose)

        # Transform points to camera space
        points_in_cam = self.transform_points_torch(points_in_world, world_to_camera)
        depth = points_in_cam[:, 2:3]

        # Project to pixels
        xy = self.ray2pixel_torch(points_in_cam)

        # Combine
        xy_and_depth = torch.cat([xy, depth], dim=-1)
